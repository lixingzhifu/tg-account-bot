...
def handle_start(message):
    ...
    markup.row("设置汇率：7.2", "设置费率：1")
    markup.row("设置佣金：1", "+1000")
    bot.send_message(message.chat.id, "欢迎使用 LX 记账机器人 ✅\n请选择或输入命令：", reply_markup=markup)
...
def set_config(message):
    ...
    for line in text.split('\n'):
        if '货币' in line:
            ...
            if result:
                currency = result[0]
        elif '汇率' in line or 'RATE' in line:
            match = re.search(r'(\d+\.?\d*)', line)
            if match:
                rate = float(match.group(1))
        elif '费率' in line or 'FEE' in line:
            match = re.search(r'(\d+\.?\d*)', line)
            if match:
                fee = float(match.group(1))
        elif '佣金' in line or 'COMM' in line:
            match = re.search(r'(\d+\.?\d*)', line)
            if match:

...
def cmd_show(msg):
    ...
    for r in rows:
        ...
        if r['action']=='deposit':
            ...
            total_dep -= r['amount']; total_pending -= r['after_fee']; total_comm -= r['commission_rmb']
        elif r['action']=='issue': total_iss += r['deducted_amount']
        elif r['action']=='delete_issue': total_iss -= r['deducted_amount']
        # 今日明细
        if local.date()==today:
...
def handle_action(msg):
    ...
    da = after if action in ('deposit','delete') else abs(after)
...
""",
            (cid, uid, msg.from_user.username, action,
             amt, after, cr, cu,
             ded, dedu,
             s['rate'], s['fee_rate'], s['commission_rate'], s['currency'])
        )
        conn.commit()
    except Exception as e:
        rollback(); return bot.reply_to(msg, f"❌ 存储失败：{e}")
    bot.reply_to(msg, f"✅ 已{ '删除' if action in ('delete','delete_issue') else '' }{ '下发' if action in ('issue','delete_issue') else '入' }款 {amt} ({s['currency']})")
